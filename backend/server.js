...
const mapFrontendToSheet = (data) => {
  ...
  return {
    ...
    '공임가격': Math.round(data.laborUnit || 0),
    '생산합계': Math.round(data.marginInfo?.totalCostUnit || data.totalCostAll || 0),
    '납품가': Math.round(data.marginInfo?.finalDeliveryUnit || data.finalDeliveryUnit || 0),
    '납품가합계': Math.round(data.marginInfo?.finalDeliveryAll || data.finalDeliveryAll || 0),
    ...
    '납품가합계+부가세': Math.round(data.marginInfo?.finalDeliveryAllVAT || data.finalDeliveryAllVAT || 0),
    '마진가격': Math.round(data.marginInfo?.marginAmountUnit || data.marginAmountUnit || 0),
    '마진%': (data.percent || 0) / 100,
    '마진합계': data.marginInfo?.marginAmountUnit ? Math.round(data.marginInfo.marginAmountUnit * (data.qty || 0)) : Math.round((data.finalDeliveryAll || 0) - ((data.totalCostAll || 0) * (data.qty || 0))),
...
const mapSheetToFrontend = (rowData) => {
  ...
  return {
    ...
    finalDeliveryUnit: parseFloat(String(rowData['납품가']).replace(/,/g, '')) || 0,
    finalDeliveryAll: parseFloat(String(rowData['납품가합계']).replace(/,/g, '')) || 0,
    finalDeliveryAllVAT: parseFloat(String(rowData['납품가합계+부가세']).replace(/,/g, '')) || 0,
    totalCostUnit: parseFloat(String(rowData['생산합계']).replace(/,/g, '')) || 0,
    marginAmountUnit: parseFloat(String(rowData['마진가격']).replace(/,/g, '')) || 0,
    marginPercent: (() => {
      const val = parseFloat(String(rowData['마진%']).replace(/,/g, '')) || 0;
      // 시트 값이 1 미만(예: 0.3)이면 100을 곱해 30%로 변환, 이미 1 이상이면 그대로 사용
